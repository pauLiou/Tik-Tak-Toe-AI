...
def draw_player_turn(screen,gs):
    ...
    if gs == True:
        text = font.render(str('Player X'), True, (0,128,0))
        text_rect = text.get_rect()
        text_rect.bottom = screen.get_rect().bottom
        text_rect.left = screen.get_rect().left
    else:
        text = font.render(str('Player O'), True, (0,128,0))
        text_rect = text.get_rect()
...
def draw_score(screen,x,o):
    font = pygame.font.SysFont(None,50)
    colour = 128
    textx = font.render(str(x.score),True,(0,colour,0))
    textx_rect = textx.get_rect()
    texto = font.render(str(o.score),True,(0,colour,0))
